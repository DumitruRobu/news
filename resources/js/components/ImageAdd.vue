...
<!--        </div>-->
        ...
        <div class="flex gap-4">
            <button class="border border-1 px-5 py-1 rounded hover:bg-[#EB7D97] hover:text-white transition duration-500">
                <router-link :to="{name:'Personal'}">Previous</router-link>
            </button>
            <button class="border border-1 px-5 py-1 rounded hover:bg-[#83B3AD] hover:text-white transition duration-500">
                <router-link :to="{name:'ReadPolicy'}">Next</router-link>
            </button>
        </div>

<!--        <button @click.prevent="registerRequest">Register</button>-->
    </div>
</template>
...
export default {
    ...
    mounted() {
        this.peopleStore.currentStep = 2;
    },
    methods:{
        updateFile(event) {
        ...
        seeTheRegisteredFile(){
            console.log(this.fileRegistered);
        },
